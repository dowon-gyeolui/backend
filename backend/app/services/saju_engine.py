...
services/saju.py 의 placeholder 를 대체하는 본격 구현.
출생 일자/시각 + 양력/음력 구분을 받아 4기둥(년주/월주/일주/시주)을
천간 + 지지 조합으로 산출한다. 다음 정통 규칙을 따른다.

  - 년주(年柱): 입춘(立春) 기준. 입춘 이전 출생은 전년 간지 사용.
  - 월주(月柱): 24절기 중 12개 절(節)이 월의 경계.
      월지(月支)는 절기 구간으로, 월간(月干)은 五虎遁法으로 년간에서 유도.
  - 일주(日柱): 1900-01-31(갑진일) 기준점에서 일수를 더해 60갑자 순환.
  - 시주(時柱): 12 시진(子=23~01, 丑=01~03, ...).
      시간(時干)은 五鼠遁法으로 일간에서 유도.

음력 입력은 korean-lunar-calendar 로 양력 변환 후 동일 로직 적용.
오행 분포는 4 천간 + 4 지지를 모두 합산한 八字 기반.
"""
