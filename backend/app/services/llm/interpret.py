...
_MODEL_DEEP = os.environ.get("OPENAI_INTERPRET_MODEL_DEEP", "gpt-5.4-mini")
...
_JAMIDUSU_SYSTEM_PROMPT = (
    ...
    "자미두수 12궁과 14주성을 **연애·연인 관점**에서 풀어주는 도우미입니다.\n"
    ...
    "공통 규칙:\n"
    + _PLAIN_KOREAN_RULES +
    "- 12궁 이름과 별 이름의 한자는 본문에서 절대 쓰지 마세요. 한국어로만 풀어 쓰세요.\n"
    ...
    "반드시 아래 JSON 스키마만 출력하십시오. 다른 설명·도입부·마크다운 금지:\n"
    "{\n"
    '  "overview": "사용자의 자미두수 전반을 연애 관점에서 2~3문장 요약",\n'
    '  "palaces": [\n'
    '    {"name": "명궁",   "description": "..."},\n'
    '    {"name": "형제궁", "description": "..."},\n'
    '    {"name": "부처궁", "description": "..."},\n'
    '    {"name": "자녀궁", "description": "..."},\n'
    '    {"name": "재백궁", "description": "..."},\n'
    '    {"name": "질액궁", "description": "..."},\n'
    '    {"name": "천이궁", "description": "..."},\n'
    '    {"name": "교우궁", "description": "..."},\n'
    '    {"name": "관록궁", "description": "..."},\n'
    '    {"name": "전택궁", "description": "..."},\n'
    '    {"name": "복덕궁", "description": "..."},\n'
    '    {"name": "부모궁", "description": "..."}\n'
    '  ],\n'
...
_JAMIDUSU_DEEP_SYSTEM_PROMPT = (
    ...
    "- 별 이름(예: 태양의 별, 재상의 별, 재고의 별 등)은 절대 언급하지 마라. 별의 기운을 쉬운 연애 표현으로만 풀어 써라.\n"
    "- 한자는 어떤 형태로도 절대 사용하지 마라.\n"
    "- love_interpretation은 2~3문장(60~130자)으로 간결하게 써라. 길게 늘이지 마라.\n"
...
def _build_jamidusu_deep_message(
    ...
    for p in chart["palaces"]:
        ...
        for s in p["stars"]:
            if s["type"] == "transform":
                stars_desc.append(f"{s['name_ko']}({s.get('sub') or ''})")
            else:
                stars_desc.append(s['name_ko'])
