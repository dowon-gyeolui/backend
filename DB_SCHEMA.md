...
erDiagram
    ...
    users ||--o{ card_unlocks  : "카드 열람=오늘의 인연(본인/상대)"
...
### 🔓 `card_unlocks` — 인연 카드 열람 기록 (오늘의 인연 + 추가 열람)
...
| `card_unlocks` | 1 | 오늘의 인연 열람 기록 (`kind='daily'`) |
...
1. ✅ **매칭 시스템 통합 완료 (2026-06-07)** — 옛 `daily_matches`(4슬롯) 시스템을 제거하고 `card_unlocks`(무료 1장 + 스타 추가 열람)로 일원화했습니다. `daily_matches` 테이블·모델·`/compatibility/{today,history,matches,score}` 엔드포인트 삭제, `delete_account`의 정리 대상도 `card_unlocks`로 교체(탈퇴 시 카드 열람 기록 정리 누락 버그도 함께 해결).
...
5. **실사용 테이블은 4개뿐** (`users`·`knowledge_chunks`·`card_unlocks`·`daily_ai_texts`) — 채팅/결제/사진/신고는 아직 데이터 0 (초기 단계).
